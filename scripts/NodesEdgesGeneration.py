...
INPUT_FILE_NAME = DIRECTORY / "JSON-output" / "<filename>.json"
OUTPUT_NODES_FILE = DIRECTORY / "csv" / "nodes.csv"
OUTPUT_EDGES_FILE = DIRECTORY / "csv" / "edges.csv"
...
def clean_json_data_by_doi(data: dict) -> dict:
    ...
    def collect_papers(paper, level=0):
        ...
        current_semantic = (paper.get("SemanticInformation") or "").strip()
        ...
        # Use the longest non-empty semantic information
        if current_semantic and len(current_semantic) > len(doi_index[doi]["SemanticInformation"]):
            doi_index[doi]["SemanticInformation"] = current_semantic
    ...
    def apply_consolidated_data(paper):
        ...
        if doi and doi in doi_index:
            ...
            if doi_index[doi]["SemanticInformation"]:
                paper["SemanticInformation"] = doi_index[doi]["SemanticInformation"]
        # Recursively process references
...
def create_graph(data: dict):
    ...
    for ref in data.get("References", []):
        ...
        nodes.append({
            ...
            "SemanticInformation": ref.get("SemanticInformation", ""),
        })
